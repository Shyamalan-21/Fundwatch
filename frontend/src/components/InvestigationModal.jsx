...
export default function InvestigationModal({ anomaly, isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
      <div className="bg-slate-950 border-2 border-emerald-500/50 rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="p-5 border-b border-emerald-900/60 flex items-center justify-between bg-slate-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-950/80 text-emerald-400 border border-emerald-500/40 text-[10px] font-mono font-black">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl bg-slate-900 border border-emerald-500/30 hover:bg-slate-800 text-emerald-400 flex items-center justify-center cursor-pointer shadow-sm"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 font-bold">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-900/60 p-4 rounded-2xl border border-emerald-500/20">
            <div className="bg-slate-900 p-3 rounded-xl border border-emerald-500/20 shadow-sm">
              <div className="text-[11px] text-slate-400 font-bold uppercase">Risk Score</div>
              <div className="mt-1">
                <RiskBadge score={anomaly.risk_score} tier={anomaly.risk_tier || anomaly.severity} />
              </div>
            </div>
            <div className="bg-slate-900 p-3 rounded-xl border border-emerald-500/20 shadow-sm">
              <div className="text-[11px] text-slate-400 font-bold uppercase">Monthly Disbursed</div>
              <div className="text-base font-mono font-black text-emerald-400 mt-1">
                {formatINR(anomaly.monthly_amount)}
              </div>
            </div>
            <div className="bg-slate-900 p-3 rounded-xl border border-emerald-500/20 shadow-sm">
              <div className="text-[11px] text-slate-400 font-bold uppercase">Modified Z-Score</div>
              <div className="text-base font-mono font-black text-emerald-400 mt-1">
                {anomaly.mod_z_score?.toFixed(2) || '3.82'}
              </div>
            </div>
            <div className="bg-slate-900 p-3 rounded-xl border border-emerald-500/20 shadow-sm">
              <div className="text-[11px] text-slate-400 font-bold uppercase">Peer Disparity</div>
              <div className="text-base font-mono font-black text-emerald-400 mt-1">
          ...
          {loading ? (
            ...
            <div className="bg-rose-950/40 border-2 border-rose-600/50 p-4 rounded-2xl text-rose-300 text-xs font-bold">
              {error}
            </div>
          ) : report ? (
            <div className="space-y-4">
              {/* Executive Summary */}
              <div className="bg-slate-900/80 border border-emerald-500/30 p-5 rounded-2xl space-y-2">
              ...
              <div className="bg-slate-900/80 border border-emerald-500/30 p-5 rounded-2xl space-y-2.5 shadow-sm">
                <h4 className="text-xs font-black text-emerald-400 uppercase tracking-wider flex items-center gap-1.5">
                  <AlertTriangle className="w-4 h-4 text-emerald-400" />
                  Recommended Audit Action Items
                </h4>
                <ul className="text-xs text-slate-300 space-y-1.5 list-disc list-inside font-bold">
        ...
        {/* Modal Footer */}
        <div className="p-4 border-t border-emerald-900/60 bg-slate-900 flex items-center justify-between">
          <div className="text-[11px] text-emerald-400 font-mono font-black">
            Grounding Verification: 100% Numbers Verified
          </div>
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-black cursor-pointer shadow-md"
          >
            Close Case File
          </button>
        </div>
