  ...
  if (!selectedAgency) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center text-slate-500">
        ...
        <button onClick={onBack} className="mt-4 px-4 py-2 bg-sky-500 text-white text-xs font-semibold rounded-lg">
          Return to Dashboard
        </button>
      </div>
    );
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-semibold text-xs shadow-lg shadow-sky-500/20 flex items-center gap-2 transition-all"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card p-4">
          ...
          <div className="text-[10px] text-slate-500 mt-0.5">Rolling median across prior months</div>
        </div>

        <div className="glass-card p-4">
          <div className="text-[11px] text-slate-400 font-medium">Flagged Month Spend ({selectedMonth})</div>
        ...
        <div className="glass-card p-4">
          <div className="text-[11px] text-slate-400 font-medium">Robust Modified Z-Score</div>
          <div className="text-xl font-bold font-mono text-purple-400 mt-1">
            {selectedAgency.signals?.modified_z_score} MAD
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5">Threshold: &gt;3.5 is severe anomaly</div>
        </div>

        <div className="glass-card p-4">
          <div className="text-[11px] text-slate-400 font-medium">Peer Comparison ({selectedAgency.state})</div>
          <div className="text-xl font-bold font-mono text-sky-400 mt-1">
            {selectedAgency.signals?.peer_ratio}x
          </div>
          <div className="text-[10px] text-slate-500 mt-0.5">vs same-category peer median</div>
        </div>
      </div>
      ...
      <div className="glass-card p-5 border border-slate-800 space-y-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 pb-3 gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400">Focus Month:</span>
            <select
              value={selectedMonth || ''}
              onChange={(e) => handleMonthChange(e.target.value)}
              className="px-2.5 py-1 bg-slate-950 border border-slate-800 rounded-lg text-xs font-mono text-sky-400 focus:outline-none focus:border-sky-500"
            >
