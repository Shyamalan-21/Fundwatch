...
import { 
  Search, Filter, ArrowUpDown, TrendingUp, AlertTriangle, ShieldCheck, 
  Download, Eye, Sparkles, ChevronRight, SlidersHorizontal, MapPin, Building,
  Activity, HelpCircle
} from 'lucide-react';
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-card p-4 sm:p-5 relative overflow-hidden group">
          ...
          <div className="mt-2 text-2xl sm:text-3xl font-extrabold font-mono text-red-400 tracking-tight flex items-baseline gap-2">
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          ...
          <div className="relative w-full lg:w-96">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-all"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto justify-start lg:justify-end">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg text-xs font-medium transition-all"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-2 border-t border-slate-800/80">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-200 focus:outline-none focus:border-sky-500"
            >
              {states.map(st => (
          ...
          <div>
            <div className="flex justify-between items-center text-[11px] font-medium text-slate-400 mb-1">
              <span>Min Risk Score</span>
              <span className="font-mono text-sky-400 font-bold">{minRiskScore}+</span>
            </div>
            <input
              type="range"
              min="0"
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-bold text-slate-100">Ranked Spending-Anomaly Registry</h3>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/70 text-[11px] text-slate-400 uppercase tracking-wider border-b border-slate-800">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold cursor-pointer" onClick={() => handleSort('agency_name')}>
                  <div className="flex items-center gap-1">
                    <span>Implementing Agency</span>
                    <ArrowUpDown className="w-3 h-3 text-slate-500" />
                  </div>
                </th>
                <th className="px-4 py-3 font-semibold cursor-pointer" onClick={() => handleSort('year_month')}>
                  <div className="flex items-center gap-1">
                    <span>Month</span>
                    <ArrowUpDown className="w-3 h-3 text-slate-500" />
                  </div>
                </th>
                <th className="px-4 py-3 font-semibold text-right cursor-pointer" onClick={() => handleSort('monthly_amount')}>
                  <div className="flex items-center justify-end gap-1">
                    <span>Monthly Spend</span>
                    <ArrowUpDown className="w-3 h-3 text-slate-500" />
                  </div>
                </th>
                <th className="px-4 py-3 font-semibold text-center cursor-pointer" onClick={() => handleSort('velocity')}>
                  <div className="flex items-center justify-center gap-1">
                    <span>Velocity Surge</span>
                    <ArrowUpDown className="w-3 h-3 text-slate-500" />
                  </div>
                </th>
                <th className="px-4 py-3 font-semibold text-center cursor-pointer" onClick={() => handleSort('risk_score')}>
                  <div className="flex items-center justify-center gap-1">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="font-mono font-bold text-slate-100">{formatINR(anom.monthly_amount)}</div>
                        <div className="text-[10px] text-slate-500 font-mono">
                          Med: {formatINR(anom.historical_median_monthly_inr)}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full font-mono font-semibold text-xs ${
                          vel >= 4.0 
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-medium text-[11px] shadow-sm flex items-center gap-1 transition-all"
