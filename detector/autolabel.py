...
def process_and_autolabel_dataset(df_raw: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for idx, row in df.iterrows():
        ...
        labeled_rows.append({
            ...
            "peer_cost_ratio": peer_ratio,
            "velocity_ratio": max(1.0, 60.0 / max(1.0, float(delta_d))) if delta_d is not None and pd.notna(delta_d) and delta_d > 0 else 1.0,
            "velocity_spike_ratio": max(1.0, 60.0 / max(1.0, float(delta_d))) if delta_d is not None and pd.notna(delta_d) and delta_d > 0 else 1.0,
            "s1_cost_anomaly": s1,
            "s2_iqr_spread": s2,
